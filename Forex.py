...
def calculate_indicators(data):
    ...
    # Calculate AMA
    ama_period = 14
    data['AMA'] = data['close'].ewm(span=ama_period, adjust=False).mean()
    print(f"AMA: {data['AMA'].iloc[-1]}")
...
def get_signal(model, data):
    features = data[['RSI', 'ATR', 'AMA']].iloc[-1:].values
...
def run_strategy(model):
    for symbol in currency_pairs:
        ...
        # Print real-time AMA value and color
        ama_color = 'green' if features.iloc[-1]['AMA'] > features.iloc[-2]['AMA'] else 'red'
        print(f"AMA Value: {features.iloc[-1]['AMA']}, AMA Color: {ama_color}")
        print(f"RSI: {features.iloc[-1]['RSI']}")
        print(f"ATR: {features.iloc[-1]['ATR']}")

        signal = get_signal(model, features)
        last_close = get_historical_data(symbol, TIMEFRAME).iloc[-1]['close']

        if signal == 'buy' and ama_color == 'green':
            sl_price = last_close - (50 * features.iloc[-1]['ATR'])
            tp_price = last_close + (50 * features.iloc[-1]['ATR'])
            market_order(symbol, volume, mt5.ORDER_TYPE_BUY, sl_price, tp_price)
        elif signal == 'sell' and ama_color == 'red':
            sl_price = last_close + (50 * features.iloc[-1]['ATR'])
            tp_price = last_close - (50 * features.iloc[-1]['ATR'])
            market_order(symbol, volume, mt5.ORDER_TYPE_SELL, sl_price, tp_price)
        else:
            print(f"Analyzing {symbol} - Hold position")
        time.sleep(200)  # Wait for 5 minutes before placing the next trade
